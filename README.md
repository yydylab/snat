...
脚本会检测当前系统和防火墙软件，按出口接口维护 NAT 与端口规则，并提供初始化备份和多版本配置快照，方便在调整 VPN、内网转发或多网卡出口时安全回退。

## 功能

- 自动识别 Debian、Ubuntu、RHEL、Rocky Linux、AlmaLinux、Fedora、Alpine、Arch 等发行版
- 自动选择可用的防火墙软件：`firewalld`、`ufw`、`nftables`、`iptables`
...
1. 使用 `01` 检查系统、防火墙软件、IPv4 转发和当前接口状态。
2. 使用 `02` 选择要配置的出口接口。
